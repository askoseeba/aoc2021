...
def load_1D(fname, input_str = None, l_strip = True, r_strip = True, to_int = False, sep = '\n'):
    data = None
    if fname:
...
def load_2D(fname, input_str = None, l_strip = True, r_strip = True, padding_symbol = None, padding_width = None, symbols_to_int = False):
    if symbols_to_int:
        data = [[int(sym) for sym in list(line)] for line in load_1D(fname, input_str, l_strip, r_strip)]
        if padding_symbol:
            padding_symbol = int(padding_symbol)
    else:
        data = [list(line) for line in load_1D(fname, input_str, l_strip, r_strip)]
    if padding_symbol:
...
def load_2D_imag(fname, input_str = None, l_strip = True, r_strip = True, padding_symbol = None, padding_width = None, symbols_to_int = False):
    data = load_2D(fname, input_str, l_strip, r_strip, padding_symbol, padding_width, symbols_to_int = symbols_to_int)
...
def print_2D(data, map = None):
    for row in data:
        for el in row:
            print(el if not map else map[el], end = '')
        print()
    return
...
def slice_2D_imag(x_start, x_stop, y_start, y_stop, data, reset_index = True): # TODO: reset_index will break size_2D_imag and print_2D_imag.
    if reset_index:
        ...
        return {loc:                            sym for loc, sym in data.items() if x_start <= loc.real and loc.real < x_stop and y_start <= loc.imag and loc.imag < y_stop}
